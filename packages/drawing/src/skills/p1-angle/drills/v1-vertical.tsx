...
export function V1VerticalDrill({ delta, onAnswer, settings }: DrillProps) {
  ...
  const angleReferenceType = settings?.angleReferenceType ?? 'vertical';

  const tiltsRight = useMemo(() => Math.random() > 0.5, []);
  const trueAngle = tiltsRight ? 90 + delta : 90 - delta;

  const handleAnswer = (saysVertical: boolean) => {
    if (answered) return;
    setAnswered(true);

    const correct = !saysVertical; // line IS tilted
    ...
    // else: user taps Continue button
  };
  ...
  // Angle difference in degrees (always shown after answer)
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[70vh] p-8">
      ...
      <p className="text-lg text-neutral-200 mb-8">
        Is this line perfectly vertical?
      </p>
